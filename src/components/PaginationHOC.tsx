...
const PaginationHOC: React.FC<IPaginationHOCProps> = ({
  ...
  const shownCountNumber =
    page <= 1 ? listLength : firsCountNumber - 1 + listLength;

  return (
    <div className="paginationWrapper">
      <div>
        <p className="showCountText">
          {`Showing ${count === 1 ? count : firsCountNumber} to ${
            count === 1 ? count : shownCountNumber
          } of ${count} entries`}
        </p>
      </div>
      <Pagination
        ...
        page={page}
        count={
          count && Math.ceil(count / count === 1 ? count : limitCountNumber)
        }
        color={"primary"}
      />
    </div>
